...
def run_two_stage(
    ...
    full_cache_ttl = max(0.1, float(getattr(args, "full_cache_ttl", 1.0)))
    run_full = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--full-cache-ttl", type=float, default=0.45,
                        help="Seconds to keep full-frame detections between periodic refreshes.")
    parser.add_argument("--result-ttl", type=float, default=0.65,
                        help="Drop inference results older than this source-frame age.")
    ...
    if args.preset == "realtime":
        ...
        preset_default("full_cache_ttl", 0.35, "--full-cache-ttl")
        preset_default("tile_cache_ttl", 0.55, "--tile-cache-ttl")
        preset_default("result_ttl", 0.55, "--result-ttl")
        ...
        preset_default("full_cache_ttl", 0.40, "--full-cache-ttl")
        preset_default("tile_cache_ttl", 0.65, "--tile-cache-ttl")
        preset_default("result_ttl", 0.60, "--result-ttl")
        ...
        preset_default("tile_budget", 1, "--tile-budget")
        preset_default("main_every", 8, "--main-every")
        preset_default("full_cache_ttl", 0.45, "--full-cache-ttl")
        preset_default("tile_cache_ttl", 0.75, "--tile-cache-ttl")
        preset_default("result_ttl", 0.65, "--result-ttl")
...
def main() -> int:
    ...
    print(f"Full det:   every {args.main_every} frame(s), cache_ttl={args.full_cache_ttl:.1f}s")
    ...
    try:
        while True:
            if not paused:
                ...
                new_infer = False
                detections, tile_count, infer_time, infer_frame_time = infer_worker.get_latest()
                result_age = time.monotonic() - infer_frame_time if infer_frame_time > 0 else 999.0
                if infer_time > 0 and result_age > args.result_ttl:
                    detections = []
                    tile_count = 0
                elif infer_time != last_infer_time:
                    new_infer = True
